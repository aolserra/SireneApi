    ...
    public class UsersService : FirebaseService
    {
        ...
        public async Task<List<Usuario>> GetUsersList(string key, string keyValue)
        {
            try
            {
                ...
                if (Constants.PartialUserCodeQuery.Equals(key))
                {
                    usersList = (await client.Child(Constants.TabUsuario).OnceAsync<Usuario>()).Where(u => u.Object.CodigoUsuario.Contains(keyValue));
                }
                else if (Constants.IdComunidadeQuery.Equals(key))
                {
                    usersList = (await client.Child(Constants.TabUsuario).OnceAsync<Usuario>()).Where(u => u.Object.IdComunidadeUsu == keyValue);
                }
                else if (Constants.IdComunidadePerfilQuery.Equals(key))
                {
                    usersList = (await client.Child(Constants.TabUsuario).OnceAsync<Usuario>()).Where(u => u.Object.IdComunidadeUsu + u.Object.PerfilUsuario == keyValue);
                }
                else
                {
